...
<body id="page-top">
		...
		<div id="content-wrapper" class="d-flex flex-column">
			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<!-- DataTales Example -->
					<div class="col-sm-12">
						<input type="hidden" id="id_venta_data" value="<?php echo $_GET['id_venta'] ?>">
						<?php
						...
						?>
							...
							<input type="hidden" id="id_venta_result" name="" value="<?php $_GET['id_venta'] ?>">
							...
							<form action="modeloNuevaVenta.php" id="data_venta" method="post">
								<div class="row">
									...
									<div class="col-sm-2">
										...
										foreach ($ventas->ventaPorId(@$key['id_venta']) as $keyVenta) {
											if ($keyVenta['estado'] == 'APROBO CLIENTE') {
												...
												$permisoEstado = $keyVenta['estado'];
											}else if ($keyVenta['estado'] == 'CONFIRMO CLIENTE') {
												echo "<em style='padding:15px; background-color:mediumseagreen; color: white;'>CONFIRMO CLIENTE</em>";
												$permisoEstado = $keyVenta['estado'];
									...
									<div class="col-sm-12">
										...
										<br>
										<!-- <p style="float: right;"><strong>COPIAR Y ENVIAR AL CLIENTE <strong style="color: blue;">http://clientes.shtex.co/pdf/formato1.php?id_venta=<?php echo base64_encode($_GET['id_venta']); ?>
												</strong>
											</strong>
										</p> -->

										<input readonly style="color:white; border: 1px solid transparent;" id="textArea" value="http://clientes.shtex.co/formato_factura.php?id_venta=<?php echo base64_encode($_GET['id_venta']); ?>">
    									<a title="Dale click para copiar el link de esta orden" onclick="copyToClipBoard()" class="btn btn-primary" style="float: right;">COPIAR LINK DE ORDEN</a>
									...
									<input type="hidden" value="<?php echo $key['observacion']; ?>" id="get_observacion">
									<div class="col-sm-6">
									...
									?>
										<div class="card-body">
											<div class="table-responsive">
												...
												<form id="datosProductosSeleccionados" action="generarVenta.php?cliente=0&id_lista=0&id_venta=0&id_inventario=0&accion=eliminar_venta&id_cliente=0" method="post" onsubmit=" document.getElementById('cancelar_pedido').disabled = true;">
													<?php
													...
													<?php if ($_SESSION['id'] == 405 or $_SESSION['id'] == 407) {
		                                       
													?>
														<input type="submit" class="btn btn-danger bt-cancelar" id="cancelar_pedido" value="CANCELAR VENTA" > <br><br><br><br> <!-- cambiar hidden por submit -->
														<script>
																		let bt_enviar = document.getElementById('cancelar_pedido');
																		let preloader = document.getElementById('preloader');
																		let wrapper = document.getElementById('wrapper');
																		let html = document.getElementById('html');
																		bt_enviar.addEventListener('click', function(){
																			preloader.innerHTML = '<center><img src="../imagenes/preloader_emp.gif" alt="" style="text-aling:center; width: auto; background-repeat:no-repeat; background-position:center; background-attachment:fixed; "></center>';
													...
													<?php if ($_SESSION['typeUser'] == 2 and $permisoEstado == 'CONFIRMO CLIENTE') {
																?>
																<form action="desaprobar_pedido_cliente.php" method="POST">
																<input type="hidden"  value='cancelar_confirmo_cliente' name="accion">
																	<input type="hidden"  value="<?php echo $_GET['id_venta'] ?>" name="id_venta">
																	<input type="hidden" value="<?php echo $id_cliente ?>" name="id_cliente">
																	
																	<input class="btn btn-danger bt-generar" id="bt_enviar" type="submit" value="VOLVER A LA ORDEN">
																	<br>
																	<br>
																</form>
												...
												<table id="" class="table table-sm table-bordered" width="100%" cellspacing="0">
													...
													<tbody>
														...
														?>
															<form id="datosProductosSeleccionados" action="generarVenta.php?cliente=0&id_lista=0&id_venta=0&id_inventario=0&accion=generaVenta&id_cliente=0" method="post" onsubmit=" document.getElementById('bt_enviar').disabled = true;">
																<?php
																foreach ($inventarios->verlistadoPrendas($_GET['id_venta']) as $key) {
																...
																<?php if ($_SESSION['typeUser'] == 2 and $permisoEstado == 'CONFIRMO CLIENTE') {
																?>
																	<input class="btn btn-warning bt-generar" id="bt_enviar" type="submit" value="GENERAR VENTA" > <!-- cambiar el hidden por el submit -->
																	...
																	<script>
																		document.getElementById('enviar_wp').addEventListener('click', function() {
																			window.open('https://api.whatsapp.com/send?phone=<?php echo $clientes->traeTelefonoCliente($id_cliente); ?>&text=aqui podras ver tu pedido: http://clientes.shtex.co/formato_factura_2.php?id_venta=<?php echo base64_encode($_GET['id_venta']); ?>', 'Editar Cliente', 'width=820,height=300, top=300, left=400, location=no');
																		});
																	</script>
			...
			<h4>Observación</h4>
			<br>
			<textarea class="form-control rounded-0" id="observacion" <?php
																		if ($_SESSION['typeUser'] == 1 || $_SESSION['typeUser'] == 2 || $_SESSION['typeUser'] == 7) {
																		?> <?php
																			if ($permisoEstado != 'INICIADA') {
						?>readonly <?php }
			...
			?>
				...
				if ($permisoEstado == 'INICIADA') {
				?>
					<input type="button" class="btn btn-success" id="guardar_obervacion" value="Guardar observación" onclick="guardar_observacion()">
	...
	<script>
		$(document).ready(function() {
			document.getElementById("observacion").value = document.getElementById("get_observacion").value;
	...
	<script>
		function guardar_observacion() {
			...
			var id_venta = $('#id_venta_data').val();
			var observacion = $('#observacion').val();
			$.ajax({
				type: 'POST',
				url: '../modelo/acciones-obervaciones.php',
				data: {
