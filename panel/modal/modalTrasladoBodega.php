...
<form action="../../modelo/accionesProductos.php" method="post">
	...
	<br>
		...
		<div class="col-sm-12">
			...
			<div id="espacioTallas">
					<div class="col-sm-12">
			<legend>Traslado de inventario <?php echo $_GET['descripcion']; ?> color: <?php echo $productos->color_hexa($_GET['color']); ?></legend>
			<legend>Tallas</legend>
		</div>
		<?php 
		foreach ($inventarios->verInventarioId($_GET['id_inventario']) as $key) {
		...
		<div class="col-sm-2">
			...
			<input type="hidden" name="accion" value="nuevoTrasladoInventarioBodega">
			<input type="hidden" name="id_inventario" value="<?php echo $key['id_inventario']; ?>">
			<input type="hidden" name="referencia" value="<?php echo $key['referencia']; ?>">
            	
		</div>
        <div class="col-sm-6">
            <label for="bodega">Seleccione A donde se Traslada:</label>
        <select name="bodega" id="bodega" class="form-control" required>
                <option value="">SELECCIONE</option>
                <?php foreach($inventarios->verBodegas() as $key){
                ?>
